#+TITLE: Emacs litterate configuration
...
(autoload 'mu4e
  "/usr/local/Cellar/mu/1.8.5/share/emacs/site-lisp/mu/mu4e/mu4e.el"
...
** Layout for mu4e (1.8) :HOOK:BINDING:INACTIVE:

#+begin_src emacs-lisp

(defun my/tagify-string (string face)
  (replace-regexp-in-string "\\[\\(.+?\\)\\]"
                            #'(lambda (string)
                                (let ((text (upcase (match-string 1 string))))
...
;;         (thread (mu4e-message-field msg :thread))
         ...
         (top-spacing (if child                +0.0 -0.0))
         (bot-spacing (if (and root has-child) +0.0 +0.0))
...
;;        (propertize (my/tagify-string subject 'nano-default)
;;                    'face face-subject)
...
;;   (save-excursion
;;     (let* ((buffer-read-only)
;;            (beg (search-forward ""  (line-beginning-position 2) t))
;;            (end (search-forward "\n" (line-beginning-position 2) t))
;;            (overlays (overlays-in beg end))
;;            (marked))
;;       (dolist (overlay overlays)
;;         (when (overlay-get overlay 'my/mu4e-mark)
;;           (setq marked t)
;;           (delete-overlay overlay)))
      
;;       (unless marked
;;         (let ((overlay (make-overlay beg (- end 1))))
;;           (overlay-put overlay 'display (svg-lib-tag "ARCHIVE"))
;;           (overlay-put overlay 'mu4e-mark t))))))

#+end_src

** Layout for mu4e (1.8) :HOOK:BINDING:
