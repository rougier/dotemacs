...
Note also that the first time you will start emacs using this configuration, *it will take a long time* because a lof of packages will be dowloaded, installed and posibly compiled if you use Emacs 28 and above. At next restart, the process should be very fast (650 ms on my machine using OSX).
** Configuration :BINDING:HOOK:INACTIVE:
:PROPERTIES:
